...
class ImageEmbedding(nn.Module):
    def __init__(
        ...
        # load from huggingface
            ...
            not seg
            and any(x in model_name.lower() for x in ["dinov2"])
            ...
            self.SDPA = False
            self.model = AutoModel.from_pretrained(
                model_name, attn_implementation="eager", torch_dtype=torch.float16
            )
            self.image_processor = AutoImageProcessor.from_pretrained(model_name)
    ...
    def load_images_from_directory(self, images_path: List[str]) -> List[Image.Image]:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            futures = {
                executor.submit(load_single_image, (i, path)): i
    ...
    def forward(
        ...
        if any(x in self.model_name.lower() for x in ["mae"]):
            ...
            if isinstance(inputs, torch.Tensor):
                ...
                if any(
                    ...
                    for x in ["ibot", "dinov1", "ml-aim", "ijepa"]
                ):
        ...
        if self.output_hidden_states:
            ...
            embedding = torch.cat(
                [embedding, outputs["last_hidden_state"].unsqueeze(-1)], dim=-1
            )
            ...
            if any(x in self.model_name.lower() for x in ["ijepa"]):
                ...
                if patch_mode:
                    ...
                    cls_token = (
                        sequence_output[:, 0]
                        .unsqueeze(1)
                        .repeat(1, patch_tokens.shape[1], 1)
                    )
                    embedding = torch.cat([cls_token, patch_tokens], dim=-1)
                else:
                    if any(
                        x in self.model_name.lower() for x in ["ibot", "mae", "dinov1"]
                    ):
                        embedding = outputs
