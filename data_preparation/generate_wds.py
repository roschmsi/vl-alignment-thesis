...
def create_webdataset_from_instances_simple(
    ...
    model_names = [
        f"{name}-{get_model_name_from_path(path)}"
        for name, path in active_modalities.items()
    ]
    ...
    modality_files = {
        name: sorted(glob.glob(path)) for name, path in active_modalities.items()
    }
    ...
    try:
        ...
        for file_chunk_paths in tqdm(file_path_iterator, desc="Processing chunks"):
            ...
            tensor_chunks = {
                ...
                for name, path in zip(active_modalities.keys(), file_chunk_paths)
            }
            ...
            for i in range(num_instances_in_chunk):
                if (
                    total_samples_written > 0
                    and total_samples_written % samples_per_shard == 0
                ):
                    writer.close()
                    shard_idx += 1
                    writer = wds.TarWriter(shard_pattern % shard_idx)
    ...
    logging.info(
        f"✅ Preprocessing complete. Wrote {total_samples_written} individual samples into {shard_idx + 1} shards."
    )
