...
def calculate_total_samples(glob_path: str) -> int:
    ...
    if not files:
        raise FileNotFoundError(
            f"No files found at '{glob_path}'. Cannot determine sample count."
        )
...
def process_directory(
    modality_name: str, input_glob_path: str, base_output_dir: str, total_samples: int
):
    """
    ...
    if not files:
        logging.warning(
            f"No files found for {modality_name} at path {input_glob_path}. Skipping."
        )
        return None
    ...
    mmap_file = np.memmap(
        mmap_path, dtype=dtype, mode="w+", shape=(total_samples, *single_shape)
    )
    ...
    for file_path in tqdm(files, desc=f"Aggregating {modality_name}"):
        ...
        mmap_file[current_position : current_position + chunk_size] = (
            tensor_chunk.numpy()
        )
...
if __name__ == "__main__":
    ...
    active_paths = {
        name: path for name, path in paths_to_process.items() if path is not None
    }

    if not active_paths:
        logging.error(
            "No input paths are configured. Please set at least one path variable."
        )
    else:
