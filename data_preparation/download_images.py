...
def download_image(row, base_output_dir="/dss/mcmlscratch/07/ga27tus3"):
    # build full path
    rel_path = row["Image Path"]  # e.g. "cc3m/img123.jpg"
    fname = os.path.join(base_output_dir, rel_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download images from a CSV file containing URLs"
    )
    parser.add_argument(
        "--csv_path",
        type=str,
        required=True,
        help="Path to the CSV file containing image URLs",
    )
    parser.add_argument(
        "--start_index",
        type=int,
        default=0,
        help="Start index for processing the CSV file",
    )
    parser.add_argument(
        "--end_index",
        type=int,
        default=None,
        help="End index for processing the CSV file",
    )
    parser.add_argument(
        "--num_processes",
        type=int,
    ...
    parser.add_argument(
        "--data_name", type=str, default="training", help="Name of the dataset"
    )
    parser.add_argument(
        "--output_dir", type=str, default=".", help="Base dir for downloads"
    )
    ...
    # pass args.output_dir into your worker
    df_multiprocess(
        df=df,
    ...
    df = df_from_shelve(
        chunk_size=args.chunk_size, func=download_image, dataset_name=args.data_name
    )
    df.to_csv(
        os.path.join(args.output_dir, f"downloaded_{args.data_name}_report.csv"),
        index=False,
    )
    print("Saved report under", args.output_dir)
