...
from PIL import (
    Image,
    ImageFile,  # Import UnidentifiedImageError
            ...
            with tqdm(total=total_lines, desc="Processing rows") as pbar:
                for row in reader:
                    ...
                    if len(rows_chunk) == chunk_size:
                        ...
                        pbar.set_postfix(
                            {"exist": exist, "non_exist": non_exist}, refresh=True
                        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Filter local corrupted images and remove corresponding rows in the CSV file"
    )
    parser.add_argument(
        "--input_csv_file",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="./",
        help="Path to the base directory containing images",
    )
    parser.add_argument(
        "--chunk_size", type=int, default=5000, help="Number of images per chunk"
    )
