...
def parse_args():
    ...
    parser.add_argument(
        "--agg_mode", type=str, default="concat", help="Aggregation mode"
    )
    parser.add_argument(
        "--throughput", action="store_true", help="Calculate throughput"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def process_batch(
    data, start_index, batch_size, output_dir, encode_function, resume, throughput=False
):
    ...
    for batch_idx in tqdm(
    ...
    ):  # Use batch_size for both encoding and saving
        ...
        ]  # Use batch_size for encoding

        # Measure encoding time
        start_time = time.time()
        with torch.cuda.amp.autocast():  # Enable automatic mixed precision
            with torch.no_grad():
                batch_embeddings = encode_function(
                    batch_data
                ).cpu()  # Encode the entire batch
        end_time = time.time()
        ...
        if throughput:
            logging.info(
                f"Batch {idx} throughput: {current_throughput:.2f} samples/sec, "
...
def process_batch_image(
    data_loader, model, start_index, batch_size, output_dir, resume, throughput=False
):
    idx = start_index // batch_size
    total_time = 0
    total_samples = 0
...
def encode_text(args, sentences, start_index):
    ...
    if args.save_name:
        output_dir = os.path.join(
            ...
            model_name,
            args.data + "_" + args.source_caption + "_" + args.save_name,
        )
    else:
        output_dir = os.path.join(
            f"{args.output_dir}/tensor_data/text_embedding",
            model_name,
            args.data + "_" + args.source_caption,
        )
...
def encode_image(args, images, start_index):
    ...
    process_batch(
        ...
        args.batch_size,
        output_dir,
        model.get_visual_embeddings_from_directory,
        args.resume,
    )
...
def main():
    args = parse_args()
    sentences, image_paths = load_data(
        DATADIR[args.data], args.source_caption, args.domain
    )
    start_index = args.start_index
    end_index = (
        args.end_index if args.end_index else max(len(sentences), len(image_paths))
    )
    ...
    if args.domain == "text":
        logging.info(
            f"Encoding text data {args.data} with model {args.text_model_name} of batch size {args.batch_size}..."
        )
        ...
        logging.info(
            f"Encoding image data {args.data} with model {args.vision_model_name} of batch size {args.batch_size}..."
        )
