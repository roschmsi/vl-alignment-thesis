...
def extract_series(results_anchors, metric_key, which="ours"):
...
def plot_num_unpaired(
    results_anchors, baseline, metric_key, title, ylabel, filename=None, dpi=200
):
    xs, ys = extract_series(results_anchors, metric_key)

    x_indices = range(len(xs))

    fig, ax = plt.subplots()

    # Plot against x_indices instead of raw xs
    ax.plot(x_indices, ys, marker="o", label=OURS_LABEL, color="tab:blue")
...
if __name__ == "__main__":
    ...
    p1 = plot_num_unpaired(
        ...
        "COCO T2I Retrieval",
        "R@1",
        filename="t2i_r1_num_unpaired.png",
    )
    p2 = plot_num_unpaired(
        ...
        "COCO I2T Retrieval",
        "R@1",
        filename="i2t_r1_num_unpaired.png",
    )
